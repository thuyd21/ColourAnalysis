...
def extract_skin_color(image):
    ...
    avg_skin_color = np.mean(skin, axis=(0, 1)).astype(int)
       ...
       # Convert the average RGB color to hex code
    hex_code = "#{:02x}{:02x}{:02x}".format(*avg_skin_color)
    
    return hex_code

# Function to convert hex color code to RGB
def rgb_code(hex_code):
...
def main():
    ...
    if uploaded_file is not None:
        ...
        rgb_values = rgb_code(skin_color)

        st.write(f"The average skin color (Hexcode) in the image is: {skin_color}")
        st.write(f"The average skin color (RGB) in the image is: {rgb_values}")
